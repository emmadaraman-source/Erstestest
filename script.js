...
function btnAjout() {
    ...
    localStorage.setItem("Mtrcl",parseInt(students[students.length - 1].numero) + 1);
...
async function deleteStudent(index) {
    if (confirm("Supprimer cet étudiant ?")) {
        ...
        const res = await fetch(Ajout_mofif_webhook, {
            ...
            body: JSON.stringify({
                Matricule : students[index].Matricule,
                Code: "sup"
            })
        });
        if (!res.ok) {
...
function goToFacture(index) {
    ...
    localStorage.setItem("Index de l'etudiant selectionner type nombre", index);
